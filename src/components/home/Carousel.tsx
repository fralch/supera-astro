  ...
  return (
    <article className='relative w-full flex flex-shrink-0 overflow-hidden shadow-2xl'>
      ...
      {images.map((image, index) => (
        <figure
          key={index}
          className={`${currentIndex === index + 1 ? '' : 'hidden'}`}
          style={{ height: '500px' }}
        >
          <img
            id='contenedor_figura'
            src={image[0]}
            alt={image[1]}
          ...
          <figcaption className='absolute inset-x-0 bottom-1 md:bottom-20 md:right-auto md:left-20 z-20 p-4 font-light text-sm text-center md:text-left mx-auto tracking-widest leading-snug'>
            ...
            {image[3] && (
              <a
                ...
                target='_blank'
                rel='noreferrer'
                className='bg-texto-tertiary hover:bg-primary-450 text-white font-bold py-2 px-4 rounded-full '
              >
                Más información
              </a>
            )}
      ...
      <button
        onClick={back}
        className='absolute left-14 top-1/2 -translate-y-1/2 w-11 h-11 flex justify-center items-center rounded-full shadow-md z-10 bg-gray-800 hover:bg-gray-500'
      >
        <svg
      ...
      <button
        onClick={next}
        className='absolute right-14 top-1/2 -translate-y-1/2 w-11 h-11 flex justify-center items-center rounded-full shadow-md z-10 bg-gray-800 hover:bg-gray-500'
      >
        <svg
          xmlns='http://www.w3.org/2000/svg'
